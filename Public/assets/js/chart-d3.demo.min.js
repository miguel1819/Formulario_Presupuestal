...
handleLineChart=function() {
    ...
    nv.addGraph(function() {
        ...
        r++)e.push( {
            ...
            x: r, y: .5*Math.cos(r/10)
        }
        );
        var t=[ {
            values: e, key: "Sine Wave", color: green
        }
        , {
            values: a, key: "Cosine Wave", color: blue
        }
        ], n=nv.models.lineChart().options( {
            transitionDuration: 300, useInteractiveGuideline: !0
        }
        );
        return n.xAxis.axisLabel("Time (s)").tickFormat(d3.format(",.1f")), n.yAxis.axisLabel("Voltage (v)").tickFormat(function(e) {
            return null==e?"N/A": d3.format(",.2f")(e)
        }
        ), d3.select("#nv-line-chart").append("svg").datum(t).call(n), nv.utils.windowResize(n.update), n
    }
    )
}
...
handleBarChart=function() {
    ...
    nv.addGraph(function() {
        ...
        return a.yAxis.axisLabel("Total Sales"), a.xAxis.axisLabel("Product"), d3.select("#nv-bar-chart").append("svg").datum(e).call(a), nv.utils.windowResize(a.update), a
    }
    )
}
...
handleStackedAreaChart=function() {
    ...
    nv.addGraph(function() {
        ...
        ).showControls(!1).duration(300);
        return a.xAxis.tickFormat(function(e) {
            return d3.time.format("%x")(new Date(e))
        }
        ), a.yAxis.tickFormat(d3.format(",.4f")), d3.select("#nv-stacked-area-chart").append("svg").datum(e).transition().duration(1e3).call(a).each("start", function() {
            setTimeout(function() {
        ...
        ), nv.utils.windowResize(a.update), a
    }
    )
}

,
handleStackedBarChart=function() {
    "use strict";
    var e=[ {
        key:"Mecanicos 1",
        color:red,
    ...
    var indices=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"];
    nv.addGraph( {
        generate:function() {
            var a=nv.models.multiBarChart().stacked(!0).showControls(!1), r=d3.select("#nv-stacked-bar-chart").append("svg").datum(e,indices);
            return r.transition().duration(0).call(a), a
        }
    }
    )
}
